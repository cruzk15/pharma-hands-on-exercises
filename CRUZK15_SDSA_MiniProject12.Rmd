...
adsl_counts(wrangle_data(adsl_data))
```
...
```{r}
  ...
  adsl_mrg_cnt <- calculate_small_n_cnt(wrangle_data(adsl_data)) %>%
    left_join(calculate_Big_N_cnt(wrangle_data(adsl_data)), by = c("TRT01A", "TRT01AN")) %>%
    mutate(perc = round((n/N)*100, digits=1)) %>%
    ungroup() %>%
    select(TRT01A, SEX, perc) 
...
test_that("calculate_small_n_cnt() output has expected dim, columns, and types", {
   ...
   expect_equal(dim(calculate_small_n_cnt(test_data)), c(length(unique(c(calculate_small_n_cnt(test_data)$TRT01A,calculate_small_n_cnt(test_data)$SEX))), 4))
})
...
test_that("adsl_counts() outputs are accurate based on expected columns, dimension, types, non-missing percentages, and counts", {
   
  #test for expected columns
   expect_named(adsl_test_data, c("TRT01A", "SEX", "perc"))
  
  #test for dimension
   expect_equal(dim(adsl_test_data), c(length(unique(c(adsl_test_data$TRT01A,adsl_test_data$SEX))), 3))
   
  #test for type
   expect_type(adsl_test_data$perc, "double")
   
  #test for non-missing percentages 
   expect_equal(sum(is.na(adsl_test_data$perc)), 0)
  ...
  test_count <- adsl_test_data %>%
    group_by(TRT01A) %>% 
    summarise(perc_by_trt=sum(perc))
  
  expect_identical(aggregate_perc$x, test_count$perc_by_trt)
})
```
